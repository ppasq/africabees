...
RUN mkdir -p /usr/src/africabees

WORKDIR /usr/src/africabees
...
COPY . /usr/src/africabees

RUN chmod +x /usr/src/africabees/tasks.py \
    && chmod +x /usr/src/africabees/entrypoint.sh

# app-specific requirements
RUN pip install --upgrade --no-cache-dir --src /usr/src -r requirements.txt
RUN pip install --upgrade -e .

VOLUME /usr/src/africabees/africabees/templates

ENTRYPOINT ["/usr/src/africabees/entrypoint.sh"]
